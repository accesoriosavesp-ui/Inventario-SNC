...
function App() {
  // SE CORRIGE LA CARGA: El estado inicial ahora arranca directamente con lo guardado
  const [inventario, setInventario] = useState(() => {
    const datosGuardados = localStorage.getItem('mi_inventario_snc');
    return datosGuardados ? JSON.parse(datosGuardados) : [];
  });
  const [busqueda, setBusqueda] = useState('');
  const [fasePantalla, setFasePantalla] = useState('bienvenida');

  const guardarEnMemoria = (nuevoInventario) => {
    setInventario(nuevoInventario);
    localStorage.setItem('mi_inventario_snc', JSON.stringify(nuevoInventario));
  };
  ...
  const handleExcelImport = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        guardarEnMemoria(inventarioProcesado);
        alert(`¡Sincronización exitosa! Se cargaron ${inventarioProcesado.length} artículos.`);
      } catch (error) {
  ...
  return (
    <div style={{ fontFamily: "'Montserrat', sans-serif", backgroundColor: '#2D0208', minHeight: '100vh', color: '#ffffff' }}>
      ...
      <style>{`
        ...
        /* SE CORRIGE EL RECORTE AÑADIENDO PADDING LATERAL */
        .texto-oro-cursivo {
      ...
      {fasePantalla === 'dashboard' && (
        <div style={{ padding: '60px 40px', backgroundColor: '#2D0208', minHeight: '100vh', boxSizing: 'border-box' }}>
          <div style={{ maxWidth: '1350px', margin: '0 auto' }}>
            ...
            {/* CORREGIDA LA PALABRA "SISTEMA" A COLOR BLANCO CLARO */}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px', marginBottom: '45px' }}>
